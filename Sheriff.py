...
async def help(ctx):
    if ctx.guild.id == 645697605829001217:
        ...
        embed.add_field(name="",value="bday -- Wish a happy birthday to someone! Usage: $bday @user_mention")
        embed.add_field(name="",value="***MOD COMMANDS***",inline=False)
        embed.add_field(name="",value="ban -- Ban a user ==== Requires `Ban_members` permission",inline=False)
        embed.add_field(name="",value="kick -- Kick a user ==== Requires `Kick_members` permission")
        await ctx.send("This command is in development! Stay tuned!")
        await ctx.send(embed=embed)
